...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['new_folder'])) {
    ...
    header("Location: music.php?dir=" . urlencode($subDir));
    exit;
}
...
foreach ($entries as $e) {
    $path = "$currentDir/$e";
    if (is_file($path) && preg_match('/\.(mp3|wav|ogg|flac|m4a)$/i', $e)) {
        $audioFiles[] = $e;
    }
}
?>
...
<head>
<meta charset="utf-8">
<title>Музыкальный обменник</title>
...
<h1>Музыкальный обменник</h1>

<div class="nav">
  📂 Путь: 
  <a href="music.php">Главная</a>
  <?php
...
<table id="file-table">
  ...
  foreach ($entries as $e):
    ...
    $isAudio = preg_match('/\.(mp3|wav|ogg|flac|m4a)$/i', $e);
    $url = 'Музыка/' . ($subDir ? rawurlencode($subDir) . '/' : '') . rawurlencode($e);
  ?>
  <tr class="<?= $isAudio ? 'track' : '' ?>" data-src="<?= $isAudio ? htmlspecialchars($url) : '' ?>">
    <td><?= $isAudio ? '🎵' : '📄' ?> <?= htmlspecialchars($e) ?></td>
    <td><?= $size ?></td>
    <td><a href="<?= $url ?>" download>Скачать</a></td>
  </tr>
  <?php endforeach; ?>
</table>
...
form.addEventListener('submit',e=>{
  ...
  xhr.open('POST','music.php?dir='+encodeURIComponent('<?php echo $subDir; ?>'));
  xhr.send(fd);
});

// 🎵 Плеер
const rows = Array.from(document.querySelectorAll('tr.track'));
const player = document.getElementById('player');
let current = 0;
if (rows.length && player) {
  const tracks = rows.map(r => r.dataset.src);
